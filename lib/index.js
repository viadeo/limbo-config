...
try {
  ...
  rootConfig._external_components = bowerConfig.directory;
} catch (e) {
  // Nothing to do, use the default value
}

// Verify required directories
rootConfig.required_dirs.forEach(function (key) {
  if (!fs.existsSync(path.resolve(rootConfig[key]))) {
    throw new ConfigurationError('The application directory ' + rootConfig[key] + ' does not exist');
  }

  // TODO Is this required ?
  //rootConfig[key] = path.normalize(rootConfig[key]);
});
